export class Slider {
  ...
  thumb: HTMLElement | null = null; // 滑块元素
  track: HTMLElement | null = null; // 轨道元素
  leftTrack: HTMLElement | null = null; // 左侧轨道元素
  rightTrack: HTMLElement | null = null; // 右侧轨道元素
  tooltip: HTMLElement | null = null; // 提示文字元素
  decreaseButton: HTMLElement | null = null; // 左侧按钮元素
  increaseButton: HTMLElement | null = null; // 右侧按钮元素
  ...
  _valueToPosition(value: number) {
    // 将值转换为百分比位置
    return ((value - this.min) / (this.max - this.min)) * 100;
  }

  _positionToValue(percent: number) {
    // 将百分比位置转换为值
